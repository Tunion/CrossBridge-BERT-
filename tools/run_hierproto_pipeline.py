...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Run the hierarchical prototype experiment with isolated outputs."
    )
    p.add_argument("--config", type=str, default="configs/hierproto_seed42.yaml")
    p.add_argument("--dry-run", action="store_true")
    return p.parse_args()


def run_cmd(cmd: List[str], cwd: Path, stage: str, logs: List[Dict[str, Any]]) -> None:
    print(f"[run_hierproto_pipeline] [{stage}] RUN: {' '.join(cmd)}")
    t0 = time.time()
...
def main() -> None:
    ...
    run_args = dict(cfg.get("run_hierproto_args", {}))

    py = sys.executable
    out_tag = str(run_args.get("out_tag", "hierproto_seed42"))
    ...
    invocation_out = ROOT / f"outputs/results/run_hierproto_pipeline_{out_tag}.json"
    ...
    unsup_cmd = [
        ...
        "--hier-prototype-enable",
        "--hier-prototype-local-k",
        str(run_args.get("hier_prototype_local_k", 4)),
        "--hier-prototype-family-min-samples",
        str(run_args.get("hier_prototype_family_min_samples", 96)),
        "--hier-prototype-shrinkage-tau",
        str(run_args.get("hier_prototype_shrinkage_tau", 96.0)),
        "--hier-prototype-global-mix-floor",
        str(run_args.get("hier_prototype_global_mix_floor", 0.25)),
        "--hier-prototype-match-conf-floor",
        str(run_args.get("hier_prototype_match_conf_floor", 0.25)),
        "--seed",
    ...
    if args.dry_run:
        print("[run_hierproto_pipeline] DRY RUN ONLY")
        for cmd in [unsup_cmd, manual_cmd]:
            print(" ".join(cmd))
        return

    run_cmd(unsup_cmd, ROOT, "train_unsup_hierproto", logs)
    run_cmd(manual_cmd, ROOT, "manual_eval_hierproto", logs)
    ...
    print(f"[run_hierproto_pipeline] invocation={invocation_out}")
