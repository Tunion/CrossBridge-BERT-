...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Run the family-aware proto-margin anchor branch with isolated outputs."
    )
    p.add_argument("--config", type=str, default="configs/family_anchor_096_thr030.yaml")
    p.add_argument("--dry-run", action="store_true")
    return p.parse_args()


def run_cmd(cmd: List[str], cwd: Path, stage: str, logs: List[Dict[str, Any]]) -> None:
    print(f"[run_family_anchor_pipeline] [{stage}] RUN: {' '.join(cmd)}")
    t0 = time.time()
...
def main() -> None:
    ...
    run_args = dict(cfg.get("run_family_anchor_args", {}))

    py = sys.executable
    out_tag = str(run_args.get("out_tag", "familyanchor"))
    ...
    invocation_out = ROOT / f"outputs/results/run_family_anchor_pipeline_{out_tag}.json"
    ...
    unsup_cmd = [
        ...
        str(run_args.get("energy_anchor_metamorphic_weight", 0.12)),
        "--seed",
        str(run_args.get("seed", 42)),
    ]
    if bool(run_args.get("energy_stable_train", True)):
        unsup_cmd.append("--energy-head-stable-train")
    if bool(run_args.get("energy_family_aware_anchor", True)):
        unsup_cmd.append("--energy-head-family-aware-anchor")
    else:
        unsup_cmd.append("--no-energy-head-family-aware-anchor")
    if bool(run_args.get("energy_anchor_metamorphic_enable", False)):
        unsup_cmd.append("--energy-head-anchor-metamorphic-enable")
    else:
        unsup_cmd.append("--no-energy-head-anchor-metamorphic-enable")
    if bool(run_args.get("energy_include_metamorphic_features", True)):
        unsup_cmd.append("--energy-head-include-metamorphic-features")
    else:
        unsup_cmd.append("--no-energy-head-include-metamorphic-features")
    ...
    manual_cmd = [
        ...
        str(run_args.get("triage_gray_threshold", 0.30)),
        "--summary-out",
        manual_summary,
        "--risk-out",
        manual_risk,
    ...
    if args.dry_run:
        print("[run_family_anchor_pipeline] DRY RUN ONLY")
        for cmd in [unsup_cmd, manual_cmd]:
            print(" ".join(cmd))
        return

    run_cmd(unsup_cmd, ROOT, "train_unsup_family_anchor", logs)
    run_cmd(manual_cmd, ROOT, "manual_eval_family_anchor", logs)
    ...
    print(f"[run_family_anchor_pipeline] invocation={invocation_out}")
