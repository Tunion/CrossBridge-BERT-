...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Run the proto-free-energy + family-aware-gray branch with isolated outputs."
    )
    p.add_argument("--config", type=str, default="configs/protofree_famgray_028.yaml")
    p.add_argument("--dry-run", action="store_true")
    return p.parse_args()


def run_cmd(cmd: List[str], cwd: Path, stage: str, logs: List[Dict[str, Any]]) -> None:
    print(f"[run_protofree_famgray_pipeline] [{stage}] RUN: {' '.join(cmd)}")
    t0 = time.time()
...
def main() -> None:
    ...
    run_args = dict(cfg.get("run_protofree_famgray_args", {}))

    py = sys.executable
    out_tag = str(run_args.get("out_tag", "protofree_famgray"))
    ...
    invocation_out = ROOT / f"outputs/results/run_protofree_famgray_pipeline_{out_tag}.json"
    ...
    unsup_cmd = [
        ...
        "--energy-head-enable",
        "--energy-head-mode",
        str(run_args.get("energy_mode", "proto_free_energy")),
        "--energy-head-loss-mode",
        str(run_args.get("energy_loss_mode", "proto_margin")),
        "--energy-head-free-energy-temp",
        str(run_args.get("energy_free_energy_temp", 0.35)),
        "--energy-head-free-energy-prior",
        str(run_args.get("energy_free_energy_prior", "uniform")),
        "--energy-head-out",
        energy_out,
    ...
    manual_cmd = [
        ...
        str(run_args.get("triage_family_min_samples", 32)),
        "--summary-out",
        manual_summary,
        "--risk-out",
        manual_risk,
    ...
    if bool(run_args.get("triage_family_aware_gray", True)):
        manual_cmd.append("--triage-family-aware-gray")
    else:
        manual_cmd.append("--no-triage-family-aware-gray")

    if args.dry_run:
        print("[run_protofree_famgray_pipeline] DRY RUN ONLY")
        for cmd in [unsup_cmd, manual_cmd]:
            print(" ".join(cmd))
        return

    run_cmd(unsup_cmd, ROOT, "train_unsup_protofree", logs)
    run_cmd(manual_cmd, ROOT, "manual_eval_protofree_famgray", logs)
    ...
    print(f"[run_protofree_famgray_pipeline] invocation={invocation_out}")
